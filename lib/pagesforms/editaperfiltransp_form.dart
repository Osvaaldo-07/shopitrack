...
class EditaPerfilTranspForm extends StatefulWidget {
  @override
  _EditaPerfilTranspFormState createState() => _EditaPerfilTranspFormState();
}

enum SingingCharacter {Mujer, Hombre, NA}

class _EditaPerfilTranspFormState extends State<EditaPerfilTranspForm> with AfterLayoutMixin {
      ...
      if (responseImg != null) {
        if (responseImg['codigo'] == 0) {
          ...
          Dialogs.dialog(context, title: 'Aviso', content: responseImg['mensaje'], actions: [
            ParamDialog(
              text: 'Aceptar',
              onPresed: () => Navigator.pushNamedAndRemoveUntil(context, HomeTranspPage.routeName, (_)=>false)
            )
          ]);
        }
        else
  ...
  _guardarCambios() async {
    ...
    setState(() {
      ...
      _valUser = _user.length!=10 ? false : true;
      if (_fnacimiento.text.length > 0)
        _valFnacimiento = Util.isValidDate(_fnacimiento.text, 'dd/MM/yyyy');
    });
    if (_formKey.currentState!.validate() && _valFnacimiento && _valUser) {
      Map<String, dynamic> data = {
        'nombre': _nombre,
        'apaterno': _apaterno,
        'amaterno': _amaterno,
        ...
        child: response==null ? Center(child: CircularProgressIndicator()) : Column(
          children: <Widget>[
            ...
            InputText(
              ...
              validator: (text) => !RegExp(r"^[a-zA-Z0-9.a-zA-Z0-9.!#$%&'*+-/=?^_`{|}~]+@[a-zA-Z0-9]+\.[a-zA-Z]+").hasMatch(text!) ? "Email inválido" : null,
              initialValue: response!['telefono'],
              maxLength: 100,
            ),
            SizedBox(height: separacion),
            Divider(
